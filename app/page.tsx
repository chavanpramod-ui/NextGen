                ...
                <ul className="flex max-h-48 flex-col overflow-y-auto">
                  {recentSearches.map((term, i) => (
                    <li key={i}>
                      <button
                        type="button"
                        className="group flex w-full items-center gap-3 rounded-lg px-3 py-2 text-left text-sm text-slate-700 transition-all duration-300 hover:translate-x-1 hover:bg-cyan-50 hover:text-cyan-700 hover:shadow-sm dark:text-slate-300 dark:hover:bg-cyan-500/10 dark:hover:text-cyan-300"
                        onClick={() => {
                          setSearchQuery(term);
