  ...
  if (!course) {
    return (
      <div className="flex min-h-screen items-center justify-center p-8 text-center text-slate-400">
        <div>
          <h2 className="mb-4 text-2xl font-semibold text-slate-100">Course not found</h2>
  ...
  return (
    <div className="mx-auto flex w-full max-w-[1500px] flex-col gap-6 px-4 py-4 sm:px-6 lg:px-8">
      ...
      <header className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <button
          onClick={() => router.push('/')}
          className="group flex w-fit items-center gap-2 rounded-lg py-2 pl-2 pr-4 text-sm font-medium text-slate-400 transition-colors hover:bg-slate-800/50 hover:text-slate-100"
        >
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_400px]">
        ...
        <div className="flex min-w-0 flex-col gap-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-start gap-6">
              ...
              <div>
                ...
                <div className="mt-4 text-sm text-slate-500">
                  Instructor: <span className="font-medium text-slate-300">{course.instructor}</span>
                </div>
              </div>
            </div>
          </motion.div>
          ...
          <motion.div 
            ...
            className="dashboard-panel aspect-video w-full overflow-hidden relative group cursor-pointer border-slate-700/50"
          >
            <div className="absolute inset-0 bg-slate-900/80 flex items-center justify-center transition-colors group-hover:bg-slate-900/60 z-10">
        ...
        <aside className="flex min-w-0 flex-col gap-6">
          ...
          >
            <h2 className="text-lg font-semibold text-slate-50">Course Progress</h2>
            <div className="mt-6">
              <div className="mb-2 flex items-center justify-between text-sm">
                <span className="text-slate-400">{course.progress}% Completed</span>
                <span className="font-medium text-slate-100">{course.progress === 100 ? 'Done' : 'In Progress'}</span>
              </div>
          ...
          >
            <div className="mb-6 flex items-center justify-between">
              <h2 className="text-lg font-semibold text-slate-50">Curriculum</h2>
              <span className="text-xs font-medium text-slate-400 uppercase tracking-wider">{course.modules.length} items</span>
            </div>
            ...
            <div className="space-y-3">
              {course.modules.map((module, idx) => (
                ...
                >
                  ...
                  <div>
                    <p className={`text-sm font-medium ${module.completed ? 'text-slate-300' : 'text-slate-200'}`}>
                      {idx + 1}. {module.title}
                    </p>
